...
"use strict";
// auxilliaries
var Extend = Object.create, PROTO = 'prototype', HAS = Object[PROTO].hasOwnProperty,
...
function update_options($select, opts)
{
    ...
    // initial selections, before ajax options were loaded, stored in mv-form- attributes
...
function ajax_dependent_controls($selects, mvform)
{
    ...
    if (selects_exist)
    {
        model.on('change', function(evt, data) {
            var request, select;
            if (!!data.key && HAS.call(dependent_selects,data.key))
            {
                ...
                ModelViewForm.doGET(select.options, request, function(success, response){
                    ...
                    if (true === do_init)
                    {
                        ...
                        {
                            if (!HAS.call(dependent_selects,k)) continue;
                            num_controls++;
    ...
    return "function" === typeof at_value
        ...
        ? function(el) {
            ...
            else
            {
                // empty, non-selected, non-checked element, bypass
                return (
                    (('checkbox' === type || 'radio' === type) && !el.checked) ||
...
function fields2model($elements, model, locale, $key, $value/*, $json_encoded*/, arrays_as_objects)
{
    ...
    for (var e=0,len=$elements.length; e<len; e++)
    {
        ...
        {
            if (
                !!(validator = el[ATTR](mvattr( 'validate' ))) || required || data_required ||
                ('email'===input_type || 'url'===input_type ||
                'datetime' === input_type || 'date' === input_type || 'time' === input_type)
            )
            {
                if (!!validator /*&& HAS.call(Validate,validator=validator[UPPER]())*/)
                {
                    ...
                    if ('BETWEEN' === validator)
                    {
                        ...
                        {
                            model.validators[ key ] = HAS.call(model.validators, key)
                    ...
                    else if (HAS.call(Validate,validator))
                    {
                        model.validators[ key ] = HAS.call(model.validators, key)
                ...
                if (!el[HAS_ATTR]('mv-model-evt'))
                    el[SET_ATTR]('mv-model-evt', "");
                if (!el[HAS_ATTR]('mv-on-model-change'))
                    el[SET_ATTR]('mv-on-model-change', "change");
                if (!el[HAS_ATTR]('mv-on-model-error'))
                    el[SET_ATTR]('mv-on-model-error', "error");
            }
        }
...
function traverse(q, o, add, key)
{
    ...
    if (!!key)
    {
        ...
        {
            for (k in o) if (HAS.call(o,k)) traverse( q, o[k], add, key + '[' + k + ']' );
        }
        else
        {
            add(q, key, o);
...
ModelViewForm[PROTO] = ModelView.Extend(Extend(Object[PROTO]), ModelView.PublishSubscribeInterface, {
    ...
    _render: function() {
        ...
        $form.modelview({
            autoSync: true,
            autobind: true,
            livebind: !!options.livebind ? 'text' : false,
            autovalidate: false,
